...
class AND_4081: public nts::IComponent {
	public:
		AND_4081(const std::string & name, const std::string & type) {
		...
		nts::Tristate	compute(std::size_t pin)  {
			if (pin == 3) {
				this->pins[3]->setValue(nts::gates::_and(this->compute(1), this->compute(2)));
				return this->pins[3]->getValue();
			}
			else if (pin == 4) {
				this->pins[4]->setValue(nts::gates::_and(this->compute(5), this->compute(6)));
				return this->pins[4]->getValue();
			}
			else if (pin == 10) {
				this->pins[10]->setValue(nts::gates::_and(this->compute(8), this->compute(9)));
				return this->pins[10]->getValue();
			}
			else if (pin == 11) {
				this->pins[11]->setValue(nts::gates::_and(this->compute(12), this->compute(13)));
